/* ----------------------------- begin view ----------------------------------*/
//console.log( "Pattern is: " + $("pattern").html());
...
var view = {
  ...
  addPopUpBlock: function (title, text) {
    $("#main").after(
      '<div id="test-modal" class="mfp-hide white-popup-block"><h1>' +
        title +
        "</h1><p>" +
        text +
        '</p><p><a class="popup-modal-dismiss" href="#">X</a></p></div>'
    );
  },
    ...
    swal(
      ...
      function (inputValue) {
        ...
        if (!controller.validateEmail(inputValue)) {
          swal.showInputError(
            "Вы где-то ошиблись. Проверьте введенные данные."
          );
          return false;
        }
        doAjaxQuery(
  ...
  showConfirm: function (bookId) {
    swal(
      ...
      function () {
        doAjaxQuery(
          "GET",
          "/admin/api/v1/books/" + bookId + "/remove",
          null,
          function (res) {
            swal(
              {
